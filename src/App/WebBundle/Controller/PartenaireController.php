...
class PartenaireController extends Controller
{
    public function newAction(UserInterface $user = null, Request $request) {
        if(is_null($user) && !$this->get('security.authorization_checker')->isGranted('ROLE_PARTENAIRE')){
            return $this->redirectToRoute('fos_user_security_login');          
        }
        if(is_null($user)){
            return $this->redirectToRoute('fos_user_security_register');          
        }
        ...
        if($request->isMethod('POST')) {
            ...
            if($form->isSubmitted()) {
                if($form->isValid()) {
                    ...
                    $em->persist($obj);
                    $em->flush();
                    $this->addFlash('info', 'Votre compte est desormais actif.');

                    $eventDispatcher = $this->get('event_dispatcher');
                    $eventDispatcher->dispatch(Events::PARTENAIRE_ADDED, new GenericEvent($obj));
        ...
        return $this->render('WebBundle/Partenaire/form.html.twig', [
    ...
    public function updateAction(UserInterface $user = null, Request $request, $slug) {
        ...
        $obj = $em->getRepository('AppCoreBundle:Partenaire')->findOneBySlug($slug);
        $form = $this->createForm(PartenaireType::class, $obj);
            ...
            if($request->isMethod('POST')) {
                ...
                if($form->isSubmitted()) {
                    if($form->isValid()) {
                        ...
                        $em->persist($obj);
                        $em->flush();
                        $this->addFlash('info', 'Votre a été modifié avec succès.');
                        return $this->redirectToRoute('homepage');
                    } else {
                        $this->addFlash('danger', 'Formulaire incorrect. Veuillez vérifier les champs.');
                    }
